...
export default function SingleParticipant(props) {
  ...
  async function handleRegister(e) {
    ...
    */
    ...
    participant.save() // This saves both duty and participant
    .then((participant)=> {
      // navigate('/excursion');
     ...
     <div>
      <Form>
        ...
        <div key={`inline-radio`} className="mb-3">
          <Form.Check inline type="radio" id={`default-radio`} name="agegroup" label='Adult' value="adult" onChange={(e) => setAgegroup(e.target.value)} />
          <Form.Check inline type="radio" id={`default-radio`} name="agegroup" label='Child' value="child" onChange={(e) => setAgegroup(e.target.value)} />
        </div>

        {agegroup === 'adult' && //inline conditional rendering
            <>
                <Form.Label>Preferred Duties</Form.Label>
                <p>Please select your favorite type of work. You will get to know what you are assigned later.</p>
                <DropdownButton id="dropdown-basic" title="Select Favorite Duty" onSelect={handleSelect} autoClose={false}>
                    <Dropdown.Item href="#/action-1" eventKey="cleaning">Cleaning</Dropdown.Item>
                    <Dropdown.Item href="#/action-2" eventKey="cooking">Cooking</Dropdown.Item>
                    <Dropdown.Item href="#/action-3" eventKey="settingATable">Setting a table</Dropdown.Item>
                    <Dropdown.Item href="#/action-4" eventKey="makingCoffeeTea">Making coffee/tea</Dropdown.Item>
                    <Dropdown.Item href="#/action-5" eventKey="shopping">Shopping</Dropdown.Item>
                    <Dropdown.Item href="#/action-6" eventKey="playingMusic">Playing Music</Dropdown.Item>
                </DropdownButton>
            </>
        }
